...
export const Layout: React.FC<LayoutProps> = ({ children, currentView, setCurrentView }) => {
  ...
  return (
    <div className="min-h-screen bg-[#f3f4f6] text-black flex font-sans">
      ...
      {/* Mobile Header */}
      <div className="md:hidden fixed top-0 left-0 right-0 bg-[#1a1a1a] text-white z-50 p-4 border-b-2 border-black flex justify-between items-center shadow-md">
      ...
      {/* Mobile Menu Overlay */}
      {isMobileMenuOpen && (
        <div className="fixed inset-0 bg-[#1a1a1a] z-40 pt-20 p-6 md:hidden">
            ...
            {NAV_ITEMS.map((item) => {
               ...
               return (
                <button
                  ...
                  className="w-full flex items-center gap-4 text-white text-xl font-bold uppercase py-4 border-b border-gray-800"
                >
                  <Icon size={24} />
                  {item.label}
                </button>
